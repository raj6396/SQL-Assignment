...
A. cust_dimen: Details of all the customers
  ...
  B. market_fact: Details of every order item sold
   ...
   C. orders_dimen: Details of every order placed
    ...
    D. prod_dimen: Details of product category and sub category
    ...
    E. shipping_dimen: Details of shipping of orders
...
/*Identify and list the Primary Keys and Foreign Keys 
for this dataset provided to you(In case you don’t find either primary or foreign key, 
then specially mention this in your answer)*/
...
/*--Write a query to display the Customer_Name and Customer_Segment 
...
/*Write a query to get the maximum and minimum sales from market_fact table*/
...
/*Display product Id and product sub category whose product category belongs to Furniture and Technlogy. 
...
GROUP BY Product_Sub_Category;
 ...
 OR Customer_Name LIKE '___D%';
 
 /*Write a SQL query to to make a list with Cust_Id, Sales, Customer Name and their region 
...
FROM (
SELECT cust_id , sum(sales) sales ,	ROW_NUMBER() OVER (ORDER BY sum(sales) DESC ) AS rnk
